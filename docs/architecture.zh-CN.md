...
Capitok 的核心意思是 `capitalize your token`：把 Agent 已经花掉的上下文和 token 成本，沉淀成可长期保存、可迁移、可恢复、可复用的资产。

从架构定位上看，Capitok 是 Agent 系统的原始对话归档与恢复底座。
它保留源对话这份“母带数据”，让后续的回放、重建索引、跨框架迁移、上层 memory 重建成为可能，而不是试图替代这些上层框架。
...
- 把 Agent 原始对话保存为长期资产，而不是一次性消耗品
- 通过独立归档层降低迁移、恢复与重建成本
- 为回放、重建索引、重建上层记忆提供基础母带
